...
{
     ret_code_t ret;
     ret = app_timer_create(&m_led_timer, APP_TIMER_MODE_REPEATED, 
                            led_flash_handler);
      APP_ERROR_CHECK(ret);
     ret = app_timer_create(&m_flash_timer, APP_TIMER_MODE_REPEATED, 
                            led_mode_handler);
      APP_ERROR_CHECK(ret);
}
...
void send_color(RGB_t* grb)
{
    ...
    for(int i=0;i<84;++i)
     {
         APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, rgb_val, 
         24, NULL, 0));
